...
class BusinessIntelligencePipeline:
    ...
    def run(
        ...
        quality_score = profile_results.get("data_quality_score", 100.0) if isinstance(profile_results, dict) else getattr(profile_results, "data_quality_score", 100.0)
        ...
        for item in raw_rules:
            if isinstance(item, dict):
                ...
                formatted_rules.append(item.model_dump())
        ...
        if hasattr(health_assessment, "model_dump"):
            health_dict = health_assessment.model_dump()
        elif hasattr(health_assessment, "dict"):
        ...
        if not logo_path:
            logger.warning(f"Brand logo image file not found! Defaulting to text header.")

        # 5. Build Risk Assessment Object
        risk_assessment = {
            "anomalies": anomalies if isinstance(anomalies, list) else (anomalies.get("anomalies", []) if isinstance(anomalies, dict) else []),
            "opportunities": insights.get("opportunities", []) if isinstance(insights, dict) else []
        }
        ...
        report_context = {
            ...
            "profile": profile_results if isinstance(profile_results, dict) else getattr(profile_results, "__dict__", {})
        }

        # 6. Render Template and Compile PDF
        if not output_pdf_filename.endswith(".pdf"):
            output_pdf_filename = f"{output_pdf_filename}.pdf"
