...
class BusinessHealthAssessment(BaseModel):
    ...
    def to_dict(self) -> Dict[str, Any]:
        """Provides dynamic compatibility across Pydantic v1 and v2."""
...
class DriverAnalysis(BaseModel):
    ...
    top_revenue_drivers: List[MetricDriver] = Field(
        ...
        description="Top key drivers influencing top-line growth."
    )
    top_profit_drivers: List[MetricDriver] = Field(
        default_factory=list, 
        description="Top key drivers influencing bottom-line margins."
    )
