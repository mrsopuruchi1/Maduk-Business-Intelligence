...
class RecommendationEngine:
    ...
    def generate(
        ...
        churn = dashboard_metrics.get('churn_rate', 0.0)
        curr_ratio = dashboard_metrics.get('current_ratio', 1.0)
        ...
        if anomaly_list:
            for anomaly in anomaly_list[:2]:
                # Handle either object attributes or dict keys safely
                metric_name = (
                    anomaly.get('metric') 
                    or getattr(anomaly, 'metric', 'Operational Anomaly')
                )
                period = (
                    anomaly.get('date_or_period') 
                    or anomaly.get('date') 
                    or getattr(anomaly, 'details', 'Recent Period')
                )
