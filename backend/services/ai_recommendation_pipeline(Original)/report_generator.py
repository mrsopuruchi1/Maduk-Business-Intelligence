...
class ReportGenerator:
    ...
    def __init__(self, template_dir: Optional[str] = None):
        ...
        # Jinja2 custom formatting filters
        self.env.filters['currency'] = lambda val: f"${val:,.2f}" if isinstance(val, (int, float)) else (val if val is not None else "$0.00")
        self.env.filters['pct'] = lambda val: f"{val:.1f}%" if isinstance(val, (int, float)) else (val if val is not None else "0.0%")
    ...
    def compile_pdf(self, html_content: str, output_path: str) -> str:
        ...
        try:
            ...
            # Fallback 3: Save HTML file if no engine is present
            fallback_html_path = output_path.replace(".pdf", ".html")
            with open(fallback_html_path, "w", encoding="utf-8") as f:
                f.write(html_content)
            logger.warning(
                f"No PDF rendering engine available. Saved HTML fallback report to '{fallback_html_path}'."
            )
