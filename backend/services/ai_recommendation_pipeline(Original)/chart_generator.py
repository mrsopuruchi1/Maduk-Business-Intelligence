    ...
    def generate_all(
        ...
        if date_col and rev_col and date_col in df:
            try:
                ...
                if prof_col and prof_col in ts_df:
        ...
        if forecasts and forecasts.get('forecast_available') and forecasts.get('dates'):
            try:
                ...
                # Compute approximate 95% confidence interval band for visual forecast bounds
                std_est = np.std(f_rev) * 0.25 if len(f_rev) > 1 else 0.0
                lower_bound = [max(0.0, val - (1.96 * std_est * (idx + 1)**0.3)) for idx, val in enumerate(f_rev)]
                upper_bound = [val + (1.96 * std_est * (idx + 1)**0.3) for idx, val in enumerate(f_rev)]
        ...
        if prod_col and prod_col in df and rev_col:
            try:
                ...
                ax.set_title("Top 5 Products by Revenue", fontsize=10, fontweight='bold', color='#1e293b', pad=10)
        ...
        if region_col and region_col in df and rev_col:
            try:
                fig, ax = plt.subplots(figsize=(6, 3), dpi=300)
        ...
        # ------------------------------------------------------------------
        if mkt_col and mkt_col in df and rev_col:
            try:
                fig, ax = plt.subplots(figsize=(6, 3), dpi=300)
                ax.scatter(df[mkt_col], df[rev_col], color=c_blue, alpha=0.7, edgecolors='#1e40af', s=35)
                
                # Add linear fit regression trendline
                if len(df.dropna(subset=[mkt_col, rev_col])) > 2:
                    m, b = np.polyfit(df[mkt_col].fillna(0), df[rev_col].fillna(0), 1)
        ...
        # ------------------------------------------------------------------
        if cust_col and cust_col in df and rev_col:
            try:
                fig, ax = plt.subplots(figsize=(6, 3), dpi=300)
                ax.scatter(df[cust_col], df[rev_col], color=c_green, alpha=0.7, edgecolors='#047857', s=35)
                
                if len(df.dropna(subset=[cust_col, rev_col])) > 2:
                    m, b = np.polyfit(df[cust_col].fillna(0), df[rev_col].fillna(0), 1)
                    x_vals = np.array(ax.get_xlim())
