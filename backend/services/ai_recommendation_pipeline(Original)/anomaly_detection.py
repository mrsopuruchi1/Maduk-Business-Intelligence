...
class AnomalyDetectionEngine:
    ...
    def detect(self, df: pd.DataFrame, mapping: Dict[str, str], kpis: Dict[str, Any] = None) -> Dict[str, Any]:
        """
            ...
            if col_name and col_name in df:
                ...
                if std_val > 0:
                    ...
                    for idx in anomalous_indices:
                        ...
                        # Categorize Risk Severity Level (Critical, High, Medium, Low)
                        ...
                        details_msg = (
                            ...
                            f"deviated significantly from normal mean (${mean_val:,.2f})."
                        )
        ...
        # 2. Multivariate Isolation Forest for General Outlier Detection
        ...
        if len(numeric_cols) >= 2 and len(df) >= 10:
            try:
                ...
                for idx in outlier_indices:
                    date_str = str(df.iloc[idx][date_col]) if date_col and date_col in df else f"Row {idx}"
                    details_msg = f"Unusual combination of operational metrics detected around period {date_str}."
        ...
        # 3. KPI Risk Checks (Liquidity, Churn, Margin)
        if kpis:
            ...
            churn = kpis.get('churn_rate', 0.0)
            if churn > 10.0:
                msg = f"Customer Churn Spike: Current churn rate of {churn:.1f}% poses revenue retention risks."
        ...
        if rev_col and rev_col in df:
            ...
            if revenue_growth > 5.0:
                opportunities.append(f"Capitalize on positive top-line growth (+{revenue_growth:.1f}%) to expand market presence.")
            
        if exp_col and exp_col in df and rev_col and rev_col in df:
            exp_ratio = df[exp_col].sum() / max(df[rev_col].sum(), 1.0)
            if exp_ratio > 0.70:
                opportunities.append("Operating expense ratio is above 70%. Vendor contract renegotiation offers 8-12% cost recovery.")
            else:
                opportunities.append("Operating margin efficiency provides dry powder for strategic customer acquisition investments.")

        mkt_col = mapping.get('marketing_spend')
        if mkt_col and mkt_col in df:
            opportunities.append("Optimize digital marketing channel allocation to improve Customer Acquisition Cost (CAC) efficiency.")

        if not opportunities:
            opportunities.append("Establish automated recurring customer feedback loops to identify expansion upsell channels.")

        # Construct Pydantic RiskAssessment Model
        ...
        logger.info(
            f"Anomaly & Risk Engine Complete. Flagged {len(raw_anomalies)} risk anomalies "
            f"and surfaced {len(opportunities)} positive business opportunities."
        )
